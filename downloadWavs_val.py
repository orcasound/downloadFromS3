...
from DateRangeHLSStream_val import DateRangeHLSStream
import datetime as dt
import os
import tempfile
import time
...
class Hydrophone(Enum):
    ...
    Notes:  1. change bucket name to audio-orcasound-net from streaming...
            2.  change url to connect at https://us-west-2.console.aws.amazon.com/s3/buckets/audio-orcasound-net
                    It was pointing to 'https://s3-us-west-2.amazonaws.com/' + hydrophone.value.bucket + '/' + hydrophone.value.ref_folder
    """

    HPhoneTup = namedtuple("Hydrophone", "name bucket ref_folder save_bucket save_folder")

    BUSH_POINT = HPhoneTup("bush_point", "audio-orcasound-net", "rpi_bush_point", "acoustic-sandbox", "ambient-sound-analysis/bush_point")
    ORCASOUND_LAB = HPhoneTup("orcasound_lab", "audio-orcasound-net", "rpi_orcasound_lab", "acoustic-sandbox", "ambient-sound-analysis/orcasound_lab")
    PORT_TOWNSEND = HPhoneTup("port_townsend", "audio-orcasound-net", "rpi_port_townsend", "acoustic-sandbox", "ambient-sound-analysis/port_townsend")
    SUNSET_BAY = HPhoneTup("sunset_bay", "audio-orcasound-net", "rpi_sunset_bay", "acoustic-sandbox", "ambient-sound-analysis/sunset_bay")
    SANDBOX = HPhoneTup("sandbox", "audio-sandbox", "ambient-sound-analysis", "acoustic-sandbox", "ambient-sound-analysis")
...
dt_start = dt.datetime(2025, 1, 5, 20, 30)
dt_stop =  dt.datetime(2025, 1, 5, 21, 0)
